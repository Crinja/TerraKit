...
namespace TerraKit
{
    ...
    public static class TerraKitNativeLibrary
    {
        ...
        // Checks whether the native library can be loaded and whether its ABI is compatible.
        public static TerraKitNativeStatus CheckAvailability()
        {
            try
            {
                // Read the native ABI version before using any other backend functions.
                ...
                if (abiVersion.Major != SupportedAbiMajor)
                {
                    return TerraKitNativeStatus.Failure(
                        TerraKitNativeStatusCode.AbiVersionMismatch,
                        "The TerraKit native library uses an incompatible ABI major version.",
                        "Unity supports ABI major " + SupportedAbiMajor +
                        ", but the loaded library reports " + abiVersion + ".",
                        abiVersion,
                        libraryVersion);
                }
    ...
    internal struct TerraKitNativeStringView
    {
        ...
        // Copies a borrowed native UTF-8 string into a managed C# string.
    ...
    // P/Invoke bindings for TerraKit's basic version and diagnostic functions.
    internal static class TerraKitNativeMethods
    {
        ...
        // Retrieves the most recent diagnostic message from the native backend.
