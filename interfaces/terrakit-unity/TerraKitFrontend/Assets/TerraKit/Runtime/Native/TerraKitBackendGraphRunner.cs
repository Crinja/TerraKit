...
{
    /// <summary>
    /// Managed copy of a mesh generated by the TerraKit backend.
    /// Native result views are borrowed, so every buffer is copied before the result is destroyed.
    /// </summary>
    ...
    {
        // Contains external generation parameters passed from Unity to the native backend.
        // These values control runtime generation but are not graph pipeline stages.
        ...
        // Default values used by the Backend Generator UI when no custom settings are provided.
    ...
    /// <summary>
    /// Translates backend graph nodes into a native TerraKit pipeline and runs one 2D region.
    /// </summary>
    public sealed class TerraKitBackendGraphRunner
    {
        public TerraKitGeneratedMeshData Generate(TerraKitGraphAsset graph)
        {
            return Generate(graph, TerraKitBackendGenerationRequest.Default);
        }

        // Generates one terrain region by compiling the graph and executing it through the backend runtime.
        public TerraKitGeneratedMeshData Generate(
            TerraKitGraphAsset graph,
            TerraKitBackendGenerationRequest generationRequest)
        {
            if (generationRequest == null)
            {
                throw new ArgumentNullException("generationRequest");
            }

            return Generate(
                graph,
                new[] { generationRequest })[0];
        }

        /// <summary>
        /// Generates multiple regions through one compiled pipeline and one native runtime.
        /// Every request must use the same cell layout and base spacing.
        /// </summary>
        // Main backend execution pipeline:
        // 1. Validate generation requests
        // 2. Compile graph into executable stages
        // 3. Create native pipeline
        // 4. Generate terrain regions
        // 5. Copy native mesh data into managed Unity objects
        public IReadOnlyList<TerraKitGeneratedMeshData> Generate(
            TerraKitGraphAsset graph,
            IReadOnlyList<TerraKitBackendGenerationRequest> generationRequests)
        {
            ...
            for (int requestIndex = 1; requestIndex < generationRequests.Count; requestIndex++)
            {
                ...
                if (candidate.CellWidth != generationRequest.CellWidth ||
            ...
            // Ensure the native TerraKit library is available before creating the backend pipeline.
            ...
            // Convert the Unity graph asset into an ordered list of executable backend stages.
            var compileResult = new TerraKitGraphCompiler().Compile(graph);
            if (!compileResult.Success)
            {
                throw new InvalidOperationException(
                    "The graph is not ready for backend generation:\n" +
                    string.Join("\n", compileResult.Errors.ToArray()));
            }

            // Current backend supports only built-in executable backend nodes.
            foreach (TerraKitNodeData node in compileResult.ExecutionOrder)
            {
                if (!TerraKitNodeRegistry.IsBackendExecutable(node.typeId))
                {
                    throw new NotSupportedException(
                        "Backend generation currently supports only Flat Height (Backend), " +
                        "Noise Height (Backend), and Heightfield Mesh (Backend). " +
                        "Unsupported node: " + node.displayName + " (" + node.typeId + ").");
                }
            }
            ...
            try
            {
                ...
                foreach (TerraKitNodeData node in compileResult.ExecutionOrder)
                {
                    ...
                    using (var typeId = new TerraKitNativeUtf8(node.typeId))
                    {
                        TerraKitNativeCall.ThrowIfFailed(
                            TerraKitNativePipelineMethods.CreateStage(
                                stageId++, typeId.View, schema.SchemaVersion, out construction),
                    ...
                    foreach (TerraKitBackendOutputPort output in schema.Outputs)
                    {
                        ...
                        if (output.ResourceKind == TerraKitBackendResourceKind.Mesh)
                        {
                            meshResourceKey = resourceKey;
                        }
                    }
                ...
                {
                    throw new InvalidOperationException(
                        "The graph did not produce a backend mesh resource.");
                }
                ...
                var layout = new TerraKitNativeRegionLayout2D
                {
                    ...
                    BaseSpacing = new TerraKitNativeVector2Float64
                    {
                        X = generationRequest.SpacingX,
                        Y = generationRequest.SpacingY
                    }
                };
                TerraKitNativeCall.ThrowIfFailed(
                    TerraKitNativeRuntimeMethods.Create2D(ref pipeline, ref layout, out runtime),
                    "tk_runtime_create_2d");

                var generatedMeshes = new List<TerraKitGeneratedMeshData>(generationRequests.Count);
                foreach (TerraKitBackendGenerationRequest regionRequest in generationRequests)
                {
                    ...
                    {
                        Seed = regionRequest.Seed,
                        RegionX = regionRequest.RegionX,
                        RegionY = regionRequest.RegionY,
                        LodLevel = regionRequest.LodLevel
                    };

                    try
                    {
                        TerraKitNativeCall.ThrowIfFailed(
                            TerraKitNativeRuntimeMethods.Generate2D(runtime, ref request, out result),
                            "tk_runtime_generate_2d");

                        TerraKitNativeTerrainMeshView meshView;
                        TerraKitNativeCall.ThrowIfFailed(
                            TerraKitNativeResultMethods.GetMesh(result, meshResourceKey, out meshView),
                            "tk_generation_result_get_mesh");
                        generatedMeshes.Add(CopyMesh(meshView));
                    }
                    finally
                    {
                        DestroyNoThrow(
                ...
                return generatedMeshes.AsReadOnly();
            }
            finally
            {
        ...
        // Connect graph node inputs to resources produced by previous backend stages.
            ...
            IDictionary<string, ulong> resourceKeys)
        {
            foreach (TerraKitBackendInputPort input in schema.Inputs)
            {
                ...
                if (edge == null)
                {
                    ...
                    throw new InvalidOperationException(
                        node.displayName + " is missing backend input " + input.DisplayName + ".");
                }
        ...
        // Convert Unity graph parameters into native backend parameter values.
        ...
        // Copies mesh buffers from native memory into managed Unity arrays.
        // Native memory is owned by TerraKit and must not be accessed after result destruction.
        private static TerraKitGeneratedMeshData CopyMesh(TerraKitNativeTerrainMeshView view)
        {
            ...
            return new TerraKitGeneratedMeshData(
                new Vector3((float)view.Origin.X, (float)view.Origin.Y, (float)view.Origin.Z),
                positions,
                indices,
                normals,
                texcoords);
        ...
        // Safely releases native resources during cleanup.
        // Errors are logged instead of interrupting shutdown.
    ...
    // Helper class for converting managed C# strings into UTF-8 memory blocks required by the C ABI.
    ...
    // Ensures C# struct layouts match the native backend ABI memory layout.
    ...
    internal static class TerraKitNativeRuntimeMethods
    {
        private const string LibraryName = "terrakit";
    ...
    internal static class TerraKitNativeResultMethods
    {
        private const string LibraryName = "terrakit";
