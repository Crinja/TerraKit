...
/// How existing height noise is modified by the amplification value.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum HeightAmplifyMode {
    /// Adds the value directly to the existing height.
    #[default]
    Add,

    /// Multiplies the existing height by the value.
    Multiply,
}
...
pub struct AmplifyHeightStage {
    ...
    input: ResourceKey,
    output: ResourceKey,
    amplify_mode: HeightAmplifyMode,
    threshold: f32,
    threshold_direction: bool,
    amplify_direction: bool,
    amplify_value: f32,
    amplify_limit: f32,
...
impl AmplifyHeightStage {
    ...
    pub const fn get_input_key(&self) -> ResourceKey {
        self.input
    }

    /// Returns the height-field key this stage creates.
    pub const fn get_output_key(&self) -> ResourceKey {
        self.output
    }

    /// Returns the configured amplification mode.
    pub const fn get_mode(&self) -> HeightAmplifyMode {
        self.amplify_mode
    }

    /// Returns the starting height threshold
    pub const fn get_threshold(&self) -> f32 {self.threshold}

    /// Returns the threshold direction boolean
    pub const fn get_threshold_dir(&self) -> bool {self.threshold_direction}

    /// Returns the amplification direction boolean
    pub const fn get_amplify_dir(&self) -> bool {self.amplify_direction}
    /// Returns the given amplification value
    pub const fn get_amplify_value(&self) -> f32 {self.amplify_value}

    /// Returns the given amplification limit
    pub const fn get_amplify_limit(&self) -> f32 {self.amplify_limit}
}
    ...
    fn execute(
        ...
        for y in 0..output_height_field.height() {
            for x in 0..output_height_field.width() {
                ...
                if !self.amplify_direction && next > self.amplify_limit{
